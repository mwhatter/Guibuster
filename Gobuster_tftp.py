...
class TFTPWindow:
    def __init__(self, root):
        ...
        self.window.title("Gobuster TFTP")
        self.window.geometry("800x600")
        self.window.configure(bg=BG_COLOR)

        title_frame = tk.Frame(self.window, bg=BG_COLOR)
        title_frame.pack(pady=10)
        tk.Label(title_frame, text="Gobuster TFTP", font=("Helvetica", 20, "bold"), fg=FG_COLOR, bg=BG_COLOR).pack()
    ...
    def create_tftp_section(self, parent_frame):
        ...
        flags_with_entries = ["-s", "--timeout", "--delay", "-o", "-p", "-t", "-w"]
...
"""
        ScrollableHelpWindow(self.root, "TFTP Help", help_text)
    ...
    def run_tftp(self):
        ...
        for flag, var in self.tftp_checkbuttons.items():
            if var.get():
                command.append(flag)
        for flag, entry in self.tftp_entries.items():
