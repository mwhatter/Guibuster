...
class DIRWindow:
    def __init__(self, root):
        ...
        self.window.title("Gobuster DIR")
        self.window.geometry("800x600")
        self.window.configure(bg=BG_COLOR)

        title_frame = tk.Frame(self.window, bg=BG_COLOR)
        title_frame.pack(pady=10)
        tk.Label(title_frame, text="Gobuster DIR", font=("Helvetica", 20, "bold"), fg=FG_COLOR, bg=BG_COLOR).pack()
    ...
    def create_dir_section(self, parent_frame):
        ...
        flags_with_entries = ["-u", "-c", "--exclude-length", "-x", "-H", "-m", "-P", "--proxy", "--timeout", "-a", "-U", "--delay", "-o", "-p", "-t", "-w"]
        flags_without_entries = ["-f", "-r", "--hide-length", "-n", "-k", "--random-agent", "--retry", "-e", "--no-tls-validation", "--no-error", "-z", "-q", "-v"]
...
"""
        ScrollableHelpWindow(self.root, "DIR Help", help_text)
