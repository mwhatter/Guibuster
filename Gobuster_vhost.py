...
class VHOSTWindow:
    def __init__(self, root):
        ...
        self.window.title("Gobuster VHOST")
        self.window.geometry("800x600")
        self.window.configure(bg=BG_COLOR)

        title_frame = tk.Frame(self.window, bg=BG_COLOR)
        title_frame.pack(pady=10)
        tk.Label(title_frame, text="Gobuster VHOST", font=("Helvetica", 20, "bold"), fg=FG_COLOR, bg=BG_COLOR).pack()
    ...
    def create_vhost_section(self, parent_frame):
        ...
        flags_with_entries = ["-u", "--append-domain", "--domain", "--exclude-length", "-H", "-m", "-P", "--proxy", "--timeout", "-a", "-U", "--delay", "-o", "-p", "-t", "-w"]
...
"""
        ScrollableHelpWindow(self.root, "VHOST Help", help_text)
